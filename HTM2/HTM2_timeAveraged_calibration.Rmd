...
In this tutorial, we are going to follow the same path as HTM0 and keep using the North Sea data set as an example.
...
Now let's take a look at the current $F_{msy}$ per species. 

Amy: Something max sus yeild? What's the F? Can you put the meaning in brackets after the term. (Of course I am not a mechanistic modeller so this is all new to me).


To do so we need to set up an ecosystem with one gear per species and vary one gear intensity at a time while the other gear stay constant (default effort value of one).

Amy: Okay gear kind of comes out of the blue here. So this is where fishing intensity gets brought in?
Also add an 's' to the third gear, e.g. 'gears'.
...
saveRDS(sim_optim2, file = "sim_optim1.2.RDS")
```

Some trade-offs here, relatively high $eRepro$ allows Sprat to coexist with the othe species but the $F_{msy}$ plot looks wrong. 

Amy: Okay, so it looks 'wrong' but why? Can you say something like:
"although it allows sprat to coexist you can see that the [whatever] assumption indicated by the graph is unrealistic (i.e. it is unlikely that sprat would [do whatever it is that looks wrong])."

The same is true for Dab but I do not want to lower too much $eRepro$ either. Instead we are going to run the $R_{max}$ calibration again to see if we improved the yield match.

Amy: (why) "I do not want to [whatever] the eRepro because this would push our assumptions of this value into unlikely realms, whereby assuming a greater proportion of eggs are [whatever] than likely"
...
sim_loop <- project(params_loop, effort = 1, t_max = 300, dt=0.1, initial_n = params_loop@initial_n ,
                      initial_n_pp = params_loop@initial_n_pp)
...
```{r step 2 - rmax calibration results, warning=F, eval = F}
