...
I read in  Naj's params here just to have a quick check at Kerala
...
library(viridis)
params@linecolour[1:23] <-plasma(23)
...
# check each species 
 plotGrowthCurves(sim,species_params(params)$species[10]) # note:  I would not expect these to exactly fit the black (emprical VB growth curve) but if they are not reaching maturation size or massively exceeding asymptotic size there's a problem


plotlySpectra(sim,power=1,total = T) # here check that  community size spectrum (black line) is continuation of the background - looks to be
...
#have a look at the params again:
species_params(params)[11,]

# very low beta value - is this realistic - large gape? also very narrow sigma value - I would use a default of 1.5 or 2 here unless it really feeds on small size range. Usually small beta accompanties large sigma though.

# ok just looke dthis fish up - can see wht this is the case. perhaps explore a slightly larger value though or thing about how this species might search differently or have lower metabolic costs (higher max consumption rate?) because it is long and thin 

# for now let's just check

params@species_params$beta[11]<- 50
params@species_params$sigma[11]<- 1

# need to rest the params (new thing that Gustav added)
...
plotGrowthCurves(sim,"Trichiurus_lepturus")

# bit better
```

You do get coexistence - is this becuase you have already calibrated Rmax using an erepro of 1?

Have a look at comparing to data:

```{r}
# function to plot with yield
...
First set up the functions we need - sorry this is still rough...
...
# err
```
(Assuming not yet calibrated to data... and that yield is in tonnes)

Now, carry out the optimisation. There are several optimastion methods to choose from - we need to select the most robust one to share here. The R package optimx seems to be the most robsut general R package and has replaced optim. 

Often this requires repeating the procedure several times. We show how this computation can been done in paralllel.



####  Optimisation 

We can optimise on multiple computer cores using optimParallel (faster).
...
params@resource_params$kappa<-316227766017 # just and example of how you can change this following any decision made with the RShiny
params<-setParams(params)
...
optim_result <-optimParallel(par=vary,getError,params=params,method   ="L-BFGS-B",lower=c(rep(3,nspp)),upper= c(rep(15,nspp)),
...
```{r}
  # newparams<- readRDS("optimx2_param.RDS")
  # newsim <- readRDS("optimx2_sim.RDS")

getRDI(newparams)/getRDD(newparams)

# seems like there is little density dependence 

# # if needed change erepro & plug back into model
 # newparams@species_params$erepro[] <-1e-3
 # newparams <- setParams(newparams)
 # newsim <- project(newparams, effort = 1, t_max = 500, dt=0.1)
 # plot(newsim)
...
# compare with yield data again

plotPredObsYield(newsim) 
# check other plots

## WHAT ARE THE YIELD UNITS? CHEKC AND FIX ABOVE

plotlyGrowthCurves(newsim,percentage = T)  # check these
plotlyFeedingLevel(newsim)  # low feeding levels at large sizes for some groups
plotGrowthCurves(newsim,"Trichiurus_lepturus") # check each species 

plotlySpectra(newsim,power=1,total = T)
plotDiet(newparams,"Trichiurus_lepturus") ##  feeding too much on background resource, though can see a shift at larger sizes, possible low on food when it shifts to other fish?? although depends whethe your consider it ot be "other food" too, such as benthos (as in North Sea model.
...
#same idea: setting theta to be all 1s - to help diagnose
