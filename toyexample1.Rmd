...
4. Calibrate growth by changing the carrying capacity of the background resource spectrum ($\kappa$)
...
p <- ggplot(pred_yield) + # plot predicted and observed yields
  geom_point(aes(x = log10(value), y = log10(obs), color = species, size = species)) +
...
plotDiet2 <- function (sim, species, xlim = c(1,NA)) 
{
  params <- sim@params
...
plotFmsy <- function(params, effortRes = .2, returnData = F, speciesData = NULL)
{
...
if(!is.null(speciesData))
{
  ...
  effortSeq <- exp(seq(0,effortMax, effortRes)) -1 # every .1 takes 2 min to run, evry .2 takes 1 min but lesser resolution
...
This plot show the ratio between observed yield (from the data loaded earlier) and the yield recreated in our model. Only one Sole matches at the moment.
...
Increasing $eRepro$ is going to move the $MSY$ towards lower effort. Until now we had the same $eRepro$ for all species but we can input species-specific values to calibrate our recruitment. To do so, let's use another shiny app again. The next one allows you to change $eRepro$ one at a time and see the effect on the species' $F_{msy}$
...
sliderTextInput(
  ...
  output$distPlot <- renderPlot({
    # set up params using values given, need check and change parameter values so units work in days units 
    params_shiny@species_params$erepro[which(params_shiny@species_params$species == input$species)] <- input$erepro
   # params@species_params$Rmax <- rep(10^input$Rmax,12)
    # params_shiny <- setParams(params_shiny,kappa=10^input$kappa)
    
    #     sim_shiny <- project(params_shiny, effort = 1, t_max = 25)
    # plot(sim_shiny)
...
params_optim@species_params$erepro <- 10^(c(-1,-3,-4,-3.5,-3,-3,-2,-3,-2,-2,-2,-2))
