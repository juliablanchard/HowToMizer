...
params <- newMultispeciesParams(sparams,inter,kappa = 1e11,max_w=1e6) # inter comes with loading "mizer"
...
params@species_params[,-which(colnames(params@species_params) %in% c("sel_func","gear","interaction_resource","pred_kernel_type","m"))]
...
fMatW <- read.csv("data/fmatWeighted.csv") # Sandeel and Cod have multiple data base so average their F and weighting by SSB

# read in time-averaged  catches  
cdat <-read.csv("data/time-averaged-catches.csv") # only that one is used at the moment | catches are estimated from fMatW

# ssb
ssbDat <- read.csv("data/time-averaged-SSB.csv")
...
sim2 <- project(steadyparams, t_max = 300, effort =1)
plot(sim2)
...
getError <- function(vary,params=params,dat=cdat$Catch_0919_tonnes,env=state,data_type="catch", tol = 0.1,timetorun=10) {
...
## test it
err<-getError(vary,steadyparams)
...
```{r step 3 optimisation, message = F}
...
params@resource_params$kappa<-3.2e11 # where does this come from?
params<-setParams(params)
...
sim <- project(params, effort = 1, t_max = 100, dt=0.1,initial_n = sim@n[100,,],initial_n_pp = sim@n_pp[100,])
plot(sim)
```
...
plotPredObsYield <-function(sim,dat=cdat$Catch_0919_tonnes){
...
need size at age data from ICES website
https://www.ices.dk/data/data-portals/Pages/DATRAS.aspx
R package, Julia sends data first
