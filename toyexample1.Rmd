...
getError <- function(vary,params=params,dat=cdat$Catch_8595_tonnes,env=state,data_type="catch", tol = 0.1,timetorun=10) {
...
vals<-optimx(par=vary,getError,params=params,lower=c(rep(3,12)),upper= c(rep(20,12)), method = c("L-BFGS-B"),
...
vals2<-optimx(par=as.numeric(vals[1:12]),getError,params=params,lower=c(rep(3,12)),upper= c(rep(20,12)), method = c("L-BFGS-B"),
control = list(trace = 6, follow.on=TRUE, maximize=FALSE))

#vals2<-optim(par=as.numeric(vals[1:12]),getError,params=params,method ="L-BFGS-B",lower=c(rep(3,12)),upper= c(rep(20,12)))
...
# optimx values:
params@species_params$R_max<- 10^as.numeric(vals2[1:12])
...
plotPredObsYield <-function(sim,dat=cdat$Catch_8595){
...
plotDiet(params,"Cod") ## perhaps feeding too much on background resource, although here note that is "other food" too, such as benthos.
